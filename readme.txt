...
* **All In One:** WPOPT support: image optimization, media clean-up, database manager, database optimizer, updates manager, Server info, Server configuration manager, WordPress enhancer, caching system and HTML - JavaScript - CSS minifier.
* **Modules Oriented:** WPOPT is divided in modules, so you can handle each module on its own, and disable not necessary one to reduce memory and CPU usage.
