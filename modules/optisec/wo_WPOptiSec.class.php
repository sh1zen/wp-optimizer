...
class WO_WPOptiSec
{
    ...
    public static function server_conf($item, $mode, $settings = array(), $no_write = false)
    {
        ...
        if (WOSettings::get_option($settings, 'srv_compression') or WOSettings::get_option($settings, 'srv_browser_cache')) {

            $response = WO_UtilRule::add_rules(
            ...
            $response = WO_UtilRule::remove_rules(
                WPOPT_MARKER_BEGIN_MIME_TYPES,
                WPOPT_MARKER_END_MIME_TYPES,
                $no_write
            );
    ...
    {
        $default_lifetime = WOSettings::get_option($settings, "srv_browser_cache.lifetime_default", WEEK_IN_SECONDS);
        ...
        foreach (self::get_ext_types() as $type => $extensions) {
            ...
            $lifetime = WOSettings::get_option($settings, "srv_browser_cache.lifetime_{$type}", $default_lifetime[$type]);
            ...
            if (WOSettings::get_option($settings, 'srv_browser_cache.cache_control')) {
                ...
                $headers_rules .= "        Header set X-Powered-By \"WP-OPTIMIZER-" . WPOPT_VERSION . "\"\n";
    ...
    private static function generate_rules_compression($settings)
    {
        ...
        if (WOSettings::get_option($settings, 'srv_compression.gzip')) {
            ...
            if (version_compare(WO_UtilEnv::get_server_version(), '2.3.7', '>=')) {
        ...
        if (WOSettings::get_option($settings, 'srv_compression.brotli')) {
    ...
    private static function generate_rules_security($settings)
    {
        ...
        if (WOSettings::get_option($settings, 'srv_security.listings')) {
            $rules .= "Options All -Indexes\n";
        }

        if (WOSettings::get_option($settings, 'srv_security.http_track&trace')) {
        ...
        if (WOSettings::get_option($settings, 'srv_security.cors'))
            $rules .= "    Header set Access-Control-Allow-Origin " . get_site_url() . "\n";

        if (WOSettings::get_option($settings, 'srv_security.xss'))
            $rules .= "    Header set X-XSS-Protection \"1; mode=block\"\n";

        if (WOSettings::get_option($settings, 'srv_security.nosniff'))
            $rules .= "    Header set X-Content-Type-Options \"nosniff\"\n";

        if (WOSettings::get_option($settings, 'srv_security.noreferrer'))
            $rules .= "    Header set Referrer-Policy \"no-referrer\"\n";

        if (WOSettings::get_option($settings, 'srv_security.noframe'))
            $rules .= "    Header set X-Frame-Options \"DENY\"\n";

        if (WOSettings::get_option($settings, 'srv_security.hsts'))
            $rules .= "    Header set Strict-Transport-Security 'max-age=31536000' env=HTTPS\n";

        $rules .= "</IfModule>\n";

        if (WOSettings::get_option($settings, 'srv_security.protect_htaccess')) {
    ...
    private static function generate_rules_enhancements($settings)
    {
        ...
        if (WOSettings::get_option($settings, 'srv_enhancements.default_utf8')) {
            $rules .= "AddDefaultCharset utf-8\n";
        }

        if (WOSettings::get_option($settings, 'srv_enhancements.timezone')) {
            $rules .= "SetEnv TZ " . get_option('timezone_string') . "\n";
        }

        if (WOSettings::get_option($settings, 'srv_enhancements.keep_alive')) {
            $rules .= "<IfModule mod_headers.c>\n";
            $rules .= "     Header set Connection keep-alive\n";
            $rules .= " </IfModule>\n";
        }

        if (WOSettings::get_option($settings, 'srv_enhancements.pagespeed')) {
        ...
        if (WOSettings::get_option($settings, 'srv_enhancements.redirect_https')) {
        ...
        if (WOSettings::get_option($settings, 'srv_enhancements.follow_symlinks')) {
            $rules .= "      Options +FollowSymlinks\n";
        }

        if (WOSettings::get_option($settings, 'srv_enhancements.remove_www')) {
