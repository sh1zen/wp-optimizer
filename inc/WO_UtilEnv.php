...
class WO_UtilEnv
{
    ...
    public static function home_url_maybe_https()
    {
        ...
        return WO_UtilEnv::url_to_maybe_https($home_url);
    }
    ...
    public static function is_https()
    {
        switch (true) {
            case (isset($_SERVER['HTTPS']) and
                WO_UtilEnv::to_boolean($_SERVER['HTTPS'])):
    ...
    {
        return WO_UtilEnv::url_to_uri(site_url()) . '/';
    }
    ...
    public static function home_url_host()
    {
        ...
        return WO_UtilEnv::host();
    }

    public static function host()
    {
        $host_port = WO_UtilEnv::host_port();
    ...
    {
        return WO_UtilEnv::url_to_uri(get_home_url()) . '/';
    }
    ...
    {
        if (WO_UtilEnv::is_url($file)) {
            if (strstr($file, '?') === false) {
                $home_url_regexp = '~' . WO_UtilEnv::home_url_regexp() . '~i';
                $file = preg_replace($home_url_regexp, '', $file);
            }
        }

        if (!WO_UtilEnv::is_url($file)) {
            $file = WO_UtilEnv::normalize_path($file);
            $file = str_replace(WO_UtilEnv::site_root(), '', $file);
            $file = ltrim($file, '/');
        }
    ...
    {
        $home_url = get_home_url();
        $regexp = WO_UtilEnv::get_url_regexp($home_url);
    ...
    public static function get_url_regexp($url)
    {
        ...
        $regexp = '(https?:)?//(www\.)?' . WO_UtilEnv::preg_quote($url);
    ...
    public static function site_root()
    {
        ...
        $site_root = WO_UtilEnv::normalize_path($site_root);
    ...
    {
        $domain_url = WO_UtilEnv::home_domain_root_url();
        return WO_UtilEnv::get_url_regexp($domain_url);
    }
    ...
    public static function url_to_docroot_filename($url)
    {
        ...
        $normalized_url = WO_UtilEnv::remove_query_all($normalized_url);
        ...
        $home_path = rtrim(WO_UtilEnv::site_path(), '/');
        ...
        $docroot = WO_UtilEnv::document_root();
    ...
    public static function document_root()
    {
        ...
        if (!empty($_SERVER['SCRIPT_FILENAME']) and
            ...
            $script_filename = WO_UtilEnv::normalize_path(
                $_SERVER['SCRIPT_FILENAME']);
            $php_self = WO_UtilEnv::normalize_path(
                $_SERVER['PHP_SELF']);
        ...
        if (!empty($_SERVER['PATH_TRANSLATED'])) {
            $document_root = substr(
                WO_UtilEnv::normalize_path($_SERVER['PATH_TRANSLATED']),
                0,
                -strlen(WO_UtilEnv::normalize_path($_SERVER['PHP_SELF'])));
        }
        elseif (!empty($_SERVER['DOCUMENT_ROOT'])) {
            $document_root = WO_UtilEnv::normalize_path($_SERVER['DOCUMENT_ROOT']);
        }
        else {
    ...
    {
        return rtrim(WO_UtilEnv::document_root(), DIRECTORY_SEPARATOR) .
    ...
    {
        $path = WO_UtilEnv::normalize_path($path);
    ...
    {
        $relative_url = WO_UtilEnv::path_remove_dots($relative_url);

        if (version_compare(PHP_VERSION, '5.4.7') < 0) {
            if (substr($relative_url, 0, 2) == '//') {
                $relative_url =
                    (WO_UtilEnv::is_https() ? 'https' : 'http') .
                    ':' . $relative_url;
            }
        }
    ...
    {
        $url = WO_UtilEnv::url_format($url, $params);
                                      ...
                                      $skip_empty = false, $separator = '&')
    {
        if ($url != '') {
            ...
            $query = WO_UtilEnv::url_query($params);
            ...
            $query = WO_UtilEnv::url_query($params, $skip_empty, $separator);
                                     ...
                                     $separator = '&')
    {
        ...
        foreach ((array)$params as $key => $value) {
            ...
            if (is_array($value)) {
                if (count($value)) {
                    $str .= ($str != '' ? '&' : '') .
                        WO_UtilEnv::url_query($value, $skip_empty, $key);
                }
            }
            else {
                $name = '';
    ...
    {
        return WO_UtilEnv::to_boolean(ini_get('zlib.output_compression'));
    }
