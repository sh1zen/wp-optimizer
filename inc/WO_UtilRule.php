...
class WO_UtilRule
{
    ...
    {
        return WO_UtilEnv::is_wpmu() and (WO_UtilEnv::is_apache() or WO_UtilEnv::is_litespeed() or WO_UtilEnv::is_nginx());
    }
    ...
    public static function add_rules($rules, $start, $end, $order = array(), $data = false)
    {
        ...
        if (empty($rules)) {
            ...
            // rules creation mode
            $rules_missing = (strstr(WO_UtilRule::clean_rules($data), WO_UtilRule::clean_rules($rules)) === false);
        ...
        if ($replace_start !== false) {
            $data = WO_UtilRule::trim_rules(substr_replace($data, $rules, $replace_start, $replace_length));
        }
        else {
            $data = WO_UtilRule::trim_rules(rtrim($data) . "\n" . $rules);
        }
    ...
    {
        switch (true) {
            case WO_UtilEnv::is_apache():
            case WO_UtilEnv::is_litespeed():
                return WO_UtilEnv::site_path() . '.htaccess';

            case WO_UtilEnv::is_nginx():
                return WO_UtilEnv::site_path() . 'nginx.conf';
        }
    ...
    private static function clean_rules($rules)
    {
        ...
        $rules = WO_UtilRule::trim_rules($rules);
    ...
    private static function write_rules($rules)
    {
        ...
        return WODisk::write($path, $rules);
    }
    ...
    {
        return preg_match('~' . WO_UtilEnv::preg_quote($start) . "\n.*?" . WO_UtilEnv::preg_quote($end) . "\n*~s", $rules);
    }
    ...
    public static function remove_rules($start, $end, $data = false)
    {
        ...
        $data = WO_UtilRule::erase_rules($data, $start, $end);
    ...
    {
        return WO_UtilEnv::is_apache() or WO_UtilEnv::is_litespeed() or WO_UtilEnv::is_nginx() or WO_UtilEnv::is_iis();
    }
}
